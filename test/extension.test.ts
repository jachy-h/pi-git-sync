...
import { FakeExtensionApi, FakeCommandContext, FakeUi } from "./helpers/fake-pi.ts";
...
class RpcFakeUi extends FakeUi {
  ...
  async custom<T>(_renderer: unknown): Promise<T | undefined> {
    try {
      const fn = _renderer as (tui: unknown, theme: { fg: (role: string, text: string) => string }, kb: unknown, done: (val?: T) => void) => { render: (w: number) => string[] };
      const done = () => {};
...
describe("Extension registration", () => {
  ...
  it("clears pi-sync status on session_start and session_shutdown", async () => {
    ...
    expect(ctx.ui.statusUpdates.some((s) => s.key === "pi-sync" && s.value === undefined)).toBe(true);

    await api.emit("session_shutdown", {}, ctx);
    expect(ctx.ui.statusUpdates.filter((s) => s.key === "pi-sync" && s.value === undefined).length).toBe(2);
  });
});
...
describe("pisync command routing", () => {
  ...
  it("routes init without URL and notifies guidance", async () => {
    ...
    (ctx.ui as unknown as RpcFakeUi).inputResponses = ["git@github.com:test/pi-config.git"];
  ...
  it("notifies 'Init cancelled' when input is empty", async () => {
    ...
    expect(ctx.ui.notifications.some((n) => n.message.includes("Init cancelled"))).toBe(true);
  });
...
describe("debug:clear-repo command", () => {
  it("cancels when confirm is rejected", async () => {
    ...
    expect(ctx.ui.notifications.some((n) => n.message.includes("Cancelled"))).toBe(true);
  });
...
describe.sequential("Extension push command interaction flow", () => {
  ...
  it("push flow shows diff+confirmation then pushes when confirmed", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      await environment.writeAgentFile("prompts/welcome.md", "changed for push\n");
  ...
  it("calls reload when push succeeds and result.reload is true", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      await environment.writeAgentFile("prompts/welcome.md", "changed for push test\n");
...
describe.sequential("Extension pull command interaction flow", () => {
  ...
  it("pull flow asks for package approval before applying synced settings", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      expect(ctx.ui.confirmCalls[0]?.title).toContain("Approve package installation");
  ...
  it("pull flow keeps settings unchanged when package approval is cancelled", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      await expect(readFile(join(environment.agentDir, "settings.json"), "utf-8")).rejects.toThrow();
      expect(ctx.reloadCalls).toBe(0);
