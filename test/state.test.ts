...
describe.sequential("sync state persistence", () => {
  ...
  it("merges updates without losing an existing baseline", async () => {
    await withTestEnvironment(async ({ agentDir }) => {
      ...
      const updated = await updateState(agentDir, {
        pendingOperation: { type: "push-rebase-conflict", startedAt: "2026-01-01T00:00:00.000Z" },
      ...
      expect(getBaselineFile(updated, "themes/dark.json")).toEqual({ sha256: "theme", mode: 0o644 });
  ...
  it("migrates schema v2 and reconciles an equal local/repo baseline", async () => {
    await withTestEnvironment(async ({ agentDir, repoDir, writeAgentFile, writeRepoFile }) => {
      ...
      await writeFile(getStatePath(agentDir), JSON.stringify({
        schemaVersion: 2,
        repoPath: repoDir,
        branch: "main",
        lastSyncedCommit: "a".repeat(40),
      ...
      expect(migrated.files["settings.json"]).toEqual({ sha256: hash, mode: 0o644 });
      expect(migrated.pendingOperation).toMatchObject({ type: "apply-failed" });
      expect(migrated.migrationReport?.reconciled).toEqual(["settings.json"]);
      expect(JSON.parse(await readFile(getStatePath(agentDir), "utf-8")).schemaVersion).toBe(3);
