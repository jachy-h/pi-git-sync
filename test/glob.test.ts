...
import { filterAllowedFiles, isPathAllowed, minimatch, normalizePath } from "../src/glob.ts";

describe("normalizePath", () => {
  it("normalizes Windows separators, leading dot segments, duplicate separators, and trailing separators", () => {
    expect(normalizePath("./skills\\review//prompt.md/")).toBe("skills/review/prompt.md");
  });
...
describe("glob matching and precedence", () => {
  it("matches nested paths without treating a single-star pattern as recursive", () => {
    expect(minimatch("skills/review/SKILL.md", "skills/**/SKILL.md")).toBe(true);
  ...
  it("gives hard deny precedence over include and exclude precedence over include", () => {
    ...
    expect(isPathAllowed("extensions/debug.log", ["extensions/**"], ["**/*.log"])).toMatchObject({
      allowed: false,
      denied: false,
      reason: "Excluded by: **/*.log",
    });
    expect(isPathAllowed("prompts/review.md", ["extensions/**"], [])).toMatchObject({
      allowed: false,
      denied: false,
  ...
  it("filters allowlisted files and reports only hard-denied paths", () => {
    expect(filterAllowedFiles(
      ["skills/review/SKILL.md", "auth.json", "skills/tmp.log", "themes/dark.json"],
