...
async function writeSettings(
  ...
  await writeFile(join(repoDir, "sync/settings.json"), JSON.stringify({ packages: remotePackages }), "utf-8");
  await writeFile(join(agentDir, "settings.json"), JSON.stringify({ packages: localPackages }), "utf-8");
}
...
describe.sequential("package reconciliation", () => {
  it("passes package sources as a single argv value without shell interpolation", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      await writeSettings(environment.repoDir, environment.agentDir, [source], []);
  ...
  it("reinstalls changed sources and reports install failures without throwing", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      expect(result.errors).toEqual([expect.stringContaining("Failed to install npm:broken@2")]);
      ...
      expect(result.rolledBack).toEqual(expect.arrayContaining([
        "npm:broken@2",
        "npm:changed@2",
        "npm:changed@1",
      ]));
  ...
  it("reports rollback errors when the previous package cannot be restored", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      expect(result.errors).toContainEqual(expect.stringContaining("Rollback failed"));
    });
  });
  ...
  it("reports an error when the pi CLI is not available", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      await environment.writeExecutable("pi", [
        "#!/bin/sh",
        "echo 'pi: command not understood'",
        "exit 127",
      ].join("\n"));
      await writeSettings(
        environment.repoDir,
