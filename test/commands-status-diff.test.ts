...
describe.sequential("PiSyncCommands.status", () => {
  ...
  it("shows repo info, branch, and sync state when configured", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      const result = await new PiSyncCommands(environment.agentDir).status(fixture.deviceBPath);
  ...
  it("shows local-only changes as capture-pending", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      // Modify agent file
      await environment.writeAgentFile("prompts/welcome.md", "locally modified\n");
  ...
  it("includes remote ahead/behind info", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      await writeFile(join(fixture.deviceAPath, "sync/prompts/welcome.md"), "new remote\n", "utf-8");
      await runGit(fixture.deviceAPath, ["add", "--all"]);
      await runGit(fixture.deviceAPath, ["commit", "--no-gpg-sign", "-m", "Remote update"]);
      await runGit(fixture.deviceAPath, ["push", "origin", "main"]);
  ...
  it("detects repo with no origin remote", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      const result = await new PiSyncCommands(environment.agentDir).status(fixtureRepo(environment));
...
describe.sequential("PiSyncCommands.diff", () => {
  ...
  it("shows file comparison, git status, and remote changes", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      await environment.writeAgentFile("prompts/welcome.md", "modified for diff\n");

      const result = await new PiSyncCommands(environment.agentDir).diff(fixture.deviceBPath);
  ...
  it("handles repo without remote gracefully (no remote diff section)", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      await saveState(environment.agentDir, createSyncState({
        repoPath,
        files: { "prompts/welcome.md": { sha256: sha256("local only\n"), mode: 0o644 } },
      }));

      const result = await new PiSyncCommands(environment.agentDir).diff(repoPath);
