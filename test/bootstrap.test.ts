...
describe("bootstrap.sh", () => {
  ...
  it("has valid bash syntax", () => {
    const result = spawnSync("bash", ["-n", bootstrapPath], { encoding: "utf-8" });
  ...
  it("exits gracefully when pi is not available", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      await environment.writeExecutable("git", [
        "#!/bin/sh",
  ...
  it("installs the extension and leaves repo initialization to /pisync init", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      const result = spawnSync("bash", [bootstrapPath, "git@github.com:test/pi-config.git"], {
        encoding: "utf-8",
        env: {
          ...process.env,
          HOME: environment.homeDir,
      ...
      expect(result.status).toBe(0);
      expect(result.stdout).toContain("npm:@jachy/pi-git-sync@0.1.16");
      expect(result.stdout).toContain("Bootstrap complete");
      expect(result.stdout).toContain("/pisync init git@github.com:test/pi-config.git");
  ...
  it("fails when extension installation fails", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      await environment.writeExecutable("pi", [
        "#!/bin/sh",
        'if [ "$1" = "--version" ]; then echo "pi v1.0.0"; exit 0; fi',
        'echo "install failed" >&2',
        "exit 42",
      ].join("\n"));

      const result = spawnSync("bash", [bootstrapPath, "https://example.test/config.git"], {
        encoding: "utf-8",
        env: {
          ...process.env,
          HOME: environment.homeDir,
