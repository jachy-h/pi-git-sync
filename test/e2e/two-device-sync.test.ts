...
async function seedAndPush(repoPath: string, initialFiles: Record<string, string>): Promise<void> {
...
describe.sequential("Two-device sync E2E", () => {
  it("full round-trip: A pushes, B pulls, A modifies, B pulls again", async () => {
    await withTestEnvironment(async (envA) => {
      ...
      await saveState(envA.agentDir, createSyncState({
        repoPath: fixture.deviceAPath,
        files: { "prompts/welcome.md": { sha256: sha256(baselineContent), mode: 0o644 } },
      }));

      // === Step 2: A modifies and pushes ===
      await envA.writeAgentFile("prompts/welcome.md", "hello from device A — updated\n");
      await envA.writeAgentFile("themes/custom.json", JSON.stringify({ name: "custom" }));
      ...
      const { createTestEnvironment: createEnv } = await import("../helpers/temp-env.ts");
      ...
      try {
        ...
        await saveState(envB.agentDir, createSyncState({
        ...
        expect(applyResult.reload || applyResult.message.includes("up to date")).toBe(true);

        // Verify B has the updated content
        const bWelcome = await readFile(join(envB.agentDir, "prompts/welcome.md"), "utf-8");
        expect(bWelcome).toBe("hello from device A — updated\n");

        const bTheme = await readFile(join(envB.agentDir, "themes/custom.json"), "utf-8");
  ...
  it("bilateral conflict: A and B diverge, push detects conflict", async () => {
    await withTestEnvironment(async (envA) => {
      const fixture = await createGitFixture(envA.rootDir);
      ...
      try {
        ...
        const isConflict = pushB.message.includes("bilateral") ||
          pushB.message.includes("Rebase conflict");
        expect(isConflict).toBe(true);

        // B's agent should NOT have been modified
        expect(await readFile(join(envB.agentDir, "prompts/welcome.md"), "utf-8")).toBe("change from B\n");
      } finally {
        await envB.cleanup();
      }
    });
