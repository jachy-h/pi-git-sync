...
async function seedConfigRepo(repoPath: string, commitMsg = "Initialize sync config"): Promise<string> {
...
describe.sequential("PiSyncCommands.push", () => {
  it("pushes local changes to the remote and applies the result back to the agent", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      // Push
      const result = await new PiSyncCommands(environment.agentDir).push(fixture.deviceBPath);

      expect(result).toMatchObject({ reload: true, message: expect.stringContaining("Pushed successfully") });
      expect(await readFile(join(fixture.deviceBPath, "sync/prompts/welcome.md"), "utf-8")).toBe("local update\n");

      // Remote should have the new commit
      const remoteHead = (await runGit(fixture.deviceBPath, ["rev-parse", "origin/main"])).stdout;
      const localHead = (await runGit(fixture.deviceBPath, ["rev-parse", "HEAD"])).stdout;
  ...
  it("returns 'No changes to push' when there is nothing to capture", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      expect(result).toMatchObject({ reload: false, message: expect.stringContaining("No changes to push") });
    });
  });
  ...
  it("detects bilateral conflicts and blocks the push without modifying remote", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      // Remote: modify
      await writeFile(join(fixture.deviceAPath, "sync/prompts/welcome.md"), "remote change\n", "utf-8");
      ...
      const remoteHeadAfter = (await runGit(fixture.deviceAPath, ["rev-parse", "origin/main"])).stdout;
      const localHeadAfter = (await runGit(fixture.deviceAPath, ["rev-parse", "HEAD"])).stdout;
  ...
  it("blocks push when potential secrets are detected (scanSecretsBeforePush=true)", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      const secretConfig = { ...config, security: { scanSecretsBeforePush: true } };
      await mkdir(join(fixture.deviceBPath, "sync/prompts"), { recursive: true });
      await writeFile(join(fixture.deviceBPath, "pi-sync.json"), JSON.stringify(secretConfig), "utf-8");
      await writeFile(join(fixture.deviceBPath, "sync/prompts/welcome.md"), "base\n", "utf-8");
      await runGit(fixture.deviceBPath, ["add", "--all"]);
      await runGit(fixture.deviceBPath, ["commit", "--no-gpg-sign", "-m", "Init with secret scan on"]);
      ...
      await environment.writeAgentFile("prompts/welcome.md", "ghp_1234567890abcdef1234567890abcdef123456\n");
  ...
  it("blocks push when validation errors are present (e.g., corrupted JSON)", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      await mkdir(join(fixture.deviceBPath, "sync/prompts"), { recursive: true });
      await writeFile(join(fixture.deviceBPath, "pi-sync.json"), JSON.stringify(myConfig), "utf-8");
      await writeFile(join(fixture.deviceBPath, "sync/settings.json"), JSON.stringify({ packages: ["npm:@jachy/pi-git-sync"] }), "utf-8");
      await writeFile(join(fixture.deviceBPath, "sync/prompts/welcome.md"), "base\n", "utf-8");
      await runGit(fixture.deviceBPath, ["add", "--all"]);
      await runGit(fixture.deviceBPath, ["commit", "--no-gpg-sign", "-m", "Init"]);
      await runGit(fixture.deviceBPath, ["push", "origin", "main"]);

      await environment.writeAgentFile("prompts/welcome.md", "base\n");
      await environment.writeAgentFile("settings.json", JSON.stringify({ packages: ["npm:@jachy/pi-git-sync"] }));
      await saveState(environment.agentDir, createSyncState({
        repoPath: fixture.deviceBPath,
        files: {
          "prompts/welcome.md": { sha256: sha256("base\n"), mode: 0o644 },
          "settings.json": { sha256: sha256(JSON.stringify({ packages: ["npm:@jachy/pi-git-sync"] })), mode: 0o644 },
        },
      }));

      // Corrupt settings.json in the repo working tree (pre-dirty the repo)
      await writeFile(join(fixture.deviceBPath, "sync/settings.json"), "not valid json at all {{{{{", "utf-8");
...
describe.sequential("PiSyncCommands.push (rebase conflict and --continue)", () => {
  it("detects rebase conflict and sets pending operation state", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      await writeFile(join(fixture.deviceAPath, "sync/prompts/welcome.md"), "remote change for conflict\n", "utf-8");
      await runGit(fixture.deviceAPath, ["add", "--all"]);
      await runGit(fixture.deviceAPath, ["commit", "--no-gpg-sign", "-m", "Remote diverging change"]);
      await runGit(fixture.deviceAPath, ["push", "origin", "main"]);

      // Agent: modify same file with conflicting content
      await environment.writeAgentFile("prompts/welcome.md", "local conflicting change\n");
  ...
  it("push --continue returns error when there is no pending operation", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      const result = await new PiSyncCommands(environment.agentDir).push(fixture.deviceBPath, undefined, "--continue");
  ...
  it("push --continue returns error when unmerged paths still exist", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      await saveState(environment.agentDir, createSyncState({
        repoPath: fixture.deviceBPath,
        pendingOperation: { type: "push-rebase-conflict" as const, startedAt: "2026-01-01T00:00:00.000Z" },
        files: { "prompts/welcome.md": { sha256: sha256("base\n"), mode: 0o644 } },
      }));

      // Create a real conflict via merge
      await runGit(fixture.deviceBPath, ["checkout", "-b", "conflict-test"]);
      await writeFile(join(fixture.deviceBPath, "sync/prompts/welcome.md"), "branch-content\n", "utf-8");
      await runGit(fixture.deviceBPath, ["add", "--all"]);
      await runGit(fixture.deviceBPath, ["commit", "--no-gpg-sign", "-m", "Branch commit"]);

      await runGit(fixture.deviceBPath, ["checkout", "main"]);
      await writeFile(join(fixture.deviceBPath, "sync/prompts/welcome.md"), "main-content\n", "utf-8");
      await runGit(fixture.deviceBPath, ["add", "--all"]);
      await runGit(fixture.deviceBPath, ["commit", "--no-gpg-sign", "-m", "Main commit"]);
  ...
  it("push --continue after rebase resolution succeeds", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      await writeFile(join(fixture.deviceBPath, "sync/prompts/welcome.md"), "resolved content after conflict\n", "utf-8");
      await runGit(fixture.deviceBPath, ["add", "--all"]);
      await runGit(fixture.deviceBPath, ["commit", "--no-gpg-sign", "-m", "Resolved rebase conflict"]);
  ...
  it("push --continue blocks when worktree is not clean", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      // Make worktree dirty
      await writeFile(join(fixture.deviceBPath, "sync/prompts/welcome.md"), "unsaved changes\n", "utf-8");
  ...
  it("pushes successfully with a custom commit message", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      await environment.writeAgentFile("prompts/welcome.md", "custom commit message test\n");
      ...
      const log = (await runGit(fixture.deviceBPath, ["log", "--format=%s", "-1"])).stdout;
  ...
  it("handles push failure when remote is unreachable", async () => {
    await withTestEnvironment(async (environment) => {
      ...
      try {
        ...
        await runGit(fixture.deviceBPath, ["remote", "set-url", "origin", "git@ssh.invalid:nope/repo.git"]);
        ...
        await environment.writeAgentFile("prompts/welcome.md", "change for failed push\n");
