...
import type { FileComparison, FileEntry, InventoryResult } from "../src/inventory.ts";
  ...
  it("renders with pending operation and package diff", () => {
    ...
    const result = formatSyncStatusV2({
      ...
      lastSyncedCommit: null,
        lastSyncedAt: null,
        files: {},
        pendingOperation: { type: "push-rebase-conflict", startedAt: "2026-01-01T00-00-00Z" },
        lastBackup: "2026-01-01T00-00-00Z",
      },
      pkgDiff: { added: ["npm:pkg-a"], removed: [], changed: ["npm:pkg-b"], unchanged: ["npm:kept"] },
    });
...
describe("formatValidationErrors", () => {
  it("formats errors and warnings", () => {
    const errors = [
      { file: "settings.json", message: "Invalid JSON", severity: "error" as const },
      { file: "settings.json", message: "Missing pi-git-sync", severity: "warning" as const },
    ];
...
describe("formatBackupList", () => {
  it("formats a list of backups", () => {
    const backups = [
      { timestamp: "2026-01-02T00-00-00-000Z", commit: "abc1234567890", reason: "apply", operation: "apply", path: "/backups/1", files: {} },
      { timestamp: "2026-01-01T00-00-00-000Z", commit: "def4567890123", reason: "pre-rollback", operation: "push", path: "/backups/2", files: {} },
    ];
