  ...
  try {
    ...
    throw new GitCommandError(
      ...
      typeof error.code === "number"
        ? error.code
        : (error.code !== undefined ? null : null),
...
export async function gitStatus(repoPath: string, trackingBranch?: string): Promise<GitStatus> {
...
export async function gitRemoteRefExists(
  ...
  const probe = await gitProbe(repoPath, ["ls-remote", "--heads", "origin", branch]);
