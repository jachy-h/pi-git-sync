...
function validatePackageSource(source: unknown): string {
  ...
  if (/[\u0000-\u001f\u007f]/.test(source)) {
    throw new Error(`Package source contains control characters: ${JSON.stringify(source)}`);
  }
...
export function parsePackageDeclarations(settings: Record<string, unknown>): PackageDeclaration[] {
  const raw = settings.packages;
  if (raw === undefined) return [];
  if (!Array.isArray(raw)) throw new Error("settings.json packages must be an array");

  return raw.map((entry) => {
    const source = typeof entry === "string"
      ? entry
      : (typeof entry === "object" && entry !== null && !Array.isArray(entry) && "source" in entry)
        ? (entry as { source?: unknown }).source
        : undefined;
    const validated = validatePackageSource(source);
    return { source: validated, normalizedSource: normalizePackageName(validated) };
  });
}
...
export async function loadPackageTrust(agentDir: string): Promise<PackageTrustStore> {
  const path = getPackageTrustPath(agentDir);
  if (!existsSync(path)) return { schemaVersion: 1, approved: {} };
  try {
    const parsed = JSON.parse(await readFile(path, "utf-8")) as Partial<PackageTrustStore>;
    if (parsed.schemaVersion !== 1 || typeof parsed.approved !== "object" || parsed.approved === null) {
...
export async function savePackageTrust(agentDir: string, store: PackageTrustStore): Promise<void> {
  const path = getPackageTrustPath(agentDir);
  ...
  try {
    ...
    await import("node:fs/promises").then(({ rm }) => rm(temp, { force: true })).catch(() => undefined);
  }
}
...
export function planPackageChanges(
  ...
  const local = parsePackageDeclarations(localSettings);
  const remote = parsePackageDeclarations(remoteSettings);
  const localMap = new Map(local.map((entry) => [entry.normalizedSource, entry]));
  const added = remote.filter((entry) => !localMap.has(entry.normalizedSource));
  const changed: Array<{ local: PackageDeclaration; remote: PackageDeclaration }> = [];
  ...
  const required = [...added.map((entry) => entry.source), ...changed.map((entry) => entry.remote.source)]
    .filter((source) => !isBuiltInTrustedSource(source))
    .filter((source) => trustStore.approved[normalizePackageName(source)] !== source);
  return { added, changed, unchanged, approvalRequired: [...new Set(required)] };
}
...
export function approvePackagePlan(
  ...
  const missing = plan.approvalRequired.filter((source) => !approved.has(source));
...
export async function preparePackagePlan(
  ...
  const repoSettingsPath = await resolveWithinRoot(safeRoot, "settings.json", "read");
  const localSettingsPath = await resolveWithinRoot(agentDir, "settings.json", "read");
...
export async function getPackageDiff(
  ...
  const repoSettingsPath = await resolveWithinRoot(safeRoot, "settings.json", "read");
  const localSettingsPath = await resolveWithinRoot(agentDir, "settings.json", "read");
  const repoPackages = parsePackageDeclarations(await readSettingsObject(repoSettingsPath));
  const localPackages = parsePackageDeclarations(await readSettingsObject(localSettingsPath));
  const repoSet = new Set(repoPackages.map((entry) => entry.normalizedSource));
  const localSet = new Set(localPackages.map((entry) => entry.normalizedSource));

  const added = repoPackages.filter((entry) => !localSet.has(entry.normalizedSource)).map((entry) => entry.source);
  const removed = localPackages.filter((entry) => !repoSet.has(entry.normalizedSource)).map((entry) => entry.source);
  const unchanged = repoPackages.filter((entry) => localSet.has(entry.normalizedSource) &&
    localPackages.some((local) => local.source === entry.source)).map((entry) => entry.source);
  const changed: string[] = [];
  for (const remote of repoPackages) {
    const local = localPackages.find((entry) => entry.normalizedSource === remote.normalizedSource);
...
async function rollbackPackageActions(
  ...
  for (const action of [...actions].reverse()) {
    try {
      await execFileAsync("pi", ["remove", normalizePackageName(action.source)], {
        env,
        timeout: 60000,
      });
    ...
    if (action.previousSource && !isBuiltInTrustedSource(action.previousSource)) {
      try {
...
export async function executePackagePlan(
  ...
  if (!(await isPiCliAvailable())) {
    result.errors.push(`pi CLI not available. Run manually: ${toInstall.map((pkg) => `pi install ${pkg}`).join("; ")}`);
    return result;
  }

  const actions: PackageAction[] = [];
  for (const source of toInstall) {
    const changed = plan.changed.find((entry) => entry.remote.source === source);
    ...
    try {
      if (changed) {
        await execFileAsync("pi", ["remove", normalizePackageName(source)], {
          env: { ...process.env, PI_CODING_AGENT_DIR: agentDir }, timeout: 60000,
        }).catch(() => undefined);
      }
      await execFileAsync("pi", ["install", source], {
        env: { ...process.env, PI_CODING_AGENT_DIR: agentDir }, timeout: 120000,
      });
  ...
  if (result.errors.length > 0) {
    ...
    if (rollback.errors.length > 0) {
      ...
      result.errors.push(...rollback.errors.map((message) => `Rollback failed: ${message}`));
    }
  }
  ...
  if (result.errors.length === 0 && options.approval?.remember && plan.approvalRequired.length > 0) {
    const trust = await loadPackageTrust(agentDir);
