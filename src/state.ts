...
export async function loadState(agentDir: string): Promise<SyncState> {
  ...
  try {
    ...
    if (version >= CURRENT_SCHEMA_VERSION) {
      return { ...DEFAULT_STATE, ...parsed, schemaVersion: CURRENT_SCHEMA_VERSION };
    }
    ...
    return { ...DEFAULT_STATE, ...migrated, schemaVersion: CURRENT_SCHEMA_VERSION };
  } catch {
    return { ...DEFAULT_STATE };
  }
}
...
async function migrateV2ToV3(
  ...
  for (const [relativePath, baseline] of Object.entries(oldFiles)) {
    ...
    if (local.entry && remote.entry && local.entry.sha256 === remote.entry.sha256) {
      files[relativePath] = {
  ...
  return {
    ...
    ...(report.reconciled.length > 0 || report.removed.length > 0 || report.conflicts.length > 0
      ? { migrationReport: report }
      : {}),
  };
}

function isPendingOperationType(value: unknown): value is PendingOperation["type"] {
...
async function readMigrationFile(baseDir: string, relativePath: string): Promise<MigrationFileResult> {
  if (!isSafeMigrationPath(relativePath)) return { available: false, entry: null };
...
async function getMigrationRepoRoot(repoPath: unknown): Promise<string | null> {
  ...
  try {
    ...
    const manifest = JSON.parse(await readFile(manifestPath, "utf-8")) as { root?: unknown };
